...
\code{model.safetensors}。若权重不存在，系统会自动降级到 TF-IDF 或规则
...
固定案例中，TF-IDF 单独通过 7/12，BERT 单独通过 11/12，完整流程通过 12/12。该结果说明
课堂演示流程能够稳定跑通，但由于固定案例包含较多典型规则表达，不能把 12/12 解释为整体泛化
准确率。BERT 改善了英文上下文和中文短句的判断，规则校正进一步处理了“内容不错，但是考试范围
不明确”等混合评价。
